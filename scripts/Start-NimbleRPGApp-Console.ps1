<#
.SYNOPSIS
    Builds and starts Nimble RPG app for local table use, and opens it in the browser, with
    a visible PowerShell window. Right-click this file and choose "Run with PowerShell" to use it
    directly. For a version with no visible window, see Start-NimbleRPGApp-Hidden.ps1.
.PARAMETER Dev
...
.PARAMETER Hidden
    ...
    Start-NimbleRPGApp-Hidden.ps1 does this for you without needing this switch.
...
if ($lanAddresses -and -not $Dev) {
    ...
    Write-Host "  New-NetFirewallRule -DisplayName 'Nimble RPG app' -Direction Inbound -Protocol TCP -LocalPort $port -Action Allow -Profile Private" -ForegroundColor Yellow
}
